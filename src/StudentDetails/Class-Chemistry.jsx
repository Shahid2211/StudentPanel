...
import CollegeFrontPage from '../CollegeFrontPage';
...
export default function Class_Chemistry() {
  ...
  const payload = { subjectId: 3, assignmentId: 0, classId: 1, sectionId: 15, studentId: 1 };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setSnackbar({ open: true, message: 'Upload error: ' + err.message, severity: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGeneratePDF = async () => {
    try {
      const blob = await pdf(<CollegeFrontPage />).toBlob();
                          ...
                          <TableRow key={index}>
                            ...
                            <TableCell sx={{ fontSize: '1rem' }}>
                              <Button variant="contained" color="info" size="small" sx={{
                                ...
                                color: 'white', fontSize: '1rem' }} onClick={()=>{navigation('/collegeFormate')}}>
                                View
                              </Button>
                            </TableCell>
